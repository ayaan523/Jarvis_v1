...
class FunctionExecutor:
    ...
    def __init__(self):
        ...
        # Keep only local persistence managers at startup.
        self._init_managers()
    ...
    async def _async_control_light(self, params: Dict) -> Dict:
        ...
        try:
            self._load_kasa_manager()
        except Exception as error:
            return {"success": False, "message": f"Kasa manager unavailable: {error}", "data": None}
    ...
    def _run_shell_command(self, params: Dict) -> Dict:
        """Execute a shell command with strict safety gates."""
        from config import SHELL_TOOL_ENABLED
        if not SHELL_TOOL_ENABLED:
            return {"success": False, "message": "Shell tool is disabled in config. (SHELL_TOOL_ENABLED=False)", "data": None}
        ...
        match = self._dangerous_pattern(command)
        if match:
            self._pending_shell_command = {"command": command, "reason": reason}
            return {
                "success": False,
                "message": f"DANGEROUS COMMAND BLOCKED: '{command}'. Matched pattern: '{match}'. Reply yes to execute this exact command.",
                "data": {"requires_confirmation": True, "command": command},
            }
        return self._execute_shell_command(command, reason)

    def _dangerous_pattern(self, command: str):
        patterns = (
            r"\brm\s+[^\n]*-(?=[a-zA-Z]*r)(?=[a-zA-Z]*f)[a-zA-Z]+", r"\bdnf\s+remove\b", r"\bmkfs(?:\.|\b)",
            r">\s*/(?:dev|etc|bin|usr|sbin|var)/", r"\bdd\s+if=", r":\(\)\s*\{",
            r"\bsudo\b", r"\bsu\b", r"\bmv\s+/", r"\bchown\b", r"\bchmod\s+-R",
        )
        return next((pattern for pattern in patterns if re.search(pattern, command)), None)

    def confirm_pending_shell_command(self, reply: str) -> Dict | None:
        """Execute the exact pending destructive command after an unambiguous yes."""
        pending = self._pending_shell_command
        if not pending:
            return None
        if reply.strip().lower() not in {"yes", "y", "confirm"}:
            self._pending_shell_command = None
            return {"success": False, "message": "Pending shell command cancelled.", "data": None}
        self._pending_shell_command = None
        return self._execute_shell_command(pending["command"], pending["reason"])

    def _execute_shell_command(self, command: str, reason: str) -> Dict:
        log_path = Path("logs") / "shell_commands.log"
        log_path.parent.mkdir(parents=True, exist_ok=True)
        with log_path.open("a", encoding="utf-8") as log_file:
            log_file.write(f"[{datetime.now().isoformat()}] REASON: {reason} | CMD: {command}\n")
        try:
            ...
            return {
                ...
                "message": f"Command executed (Return {result.returncode}). Output: {output[:500]}",
                "data": {"stdout": stdout, "stderr": stderr, "code": result.returncode}
            }
        except subprocess.TimeoutExpired:
            return {"success": False, "message": "Command timed out after 10s", "data": None}
        except Exception as e:
            return {"success": False, "message": f"Execution error: {e}", "data": None}
            
    def _list_windows(self) -> Dict:
        """List open desktop windows."""
        import subprocess
        if self._is_wayland():
            # Wayland window listing is compositor-specific. GNOME uses DBus or extensions.
            # Return unsupported to avoid silently failing.
            return {"success": False, "message": "Window management is limited under Wayland. Cannot reliably list windows without compositor-specific DBus APIs.", "data": None}
            
        try:
            result = subprocess.run(["wmctrl", "-l"], capture_output=True, text=True, timeout=2)
            if result.returncode != 0:
                return {"success": False, "message": "Failed to list windows. Is wmctrl installed?", "data": None}
            ...
            return {"success": True, "message": f"Found {len(windows)} windows.", "data": {"windows": windows}}
        except Exception as e:
            return {"success": False, "message": f"Error listing windows: {e}", "data": None}
            
    def _focus_window(self, params: Dict) -> Dict:
        """Focus a specific window."""
        title = params.get("title", "")
        if not title:
            return {"success": False, "message": "No window title provided", "data": None}
            
        if self._is_wayland():
            return {"success": False, "message": "Focusing windows is unsupported under Wayland security model.", "data": None}
            
        import subprocess
        try:
            result = subprocess.run(["wmctrl", "-a", title], capture_output=True, text=True, timeout=2)
            if result.returncode == 0:
                return {"success": True, "message": f"Focused window matching '{title}'", "data": None}
            return {"success": False, "message": f"Could not find or focus window '{title}'", "data": None}
        except Exception as e:
            return {"success": False, "message": f"Error focusing window: {e}", "data": None}
    ...
    def _close_window(self, params: Dict) -> Dict:
        ...
        if self._is_wayland():
            return {"success": False, "message": "Closing windows is unsupported under Wayland security model.", "data": None}
            
        import subprocess
        try:
            result = subprocess.run(["wmctrl", "-c", title], capture_output=True, text=True, timeout=2)
            if result.returncode == 0:
                return {"success": True, "message": f"Closed window matching '{title}'", "data": None}
            return {"success": False, "message": f"Could not find or close window '{title}'", "data": None}
        except Exception as e:
            return {"success": False, "message": f"Error closing window: {e}", "data": None}
    ...
    def _get_system_info(self) -> Dict:
        ...
        try:
            self._load_kasa_manager()
            self._load_weather_manager()
            self._load_news_manager()
        except Exception as error:
            print(f"[FunctionExecutor] Optional system-info provider unavailable: {error}")
        info = {
...
def run_shell_command(command: str, reason: str) -> Dict:
    """Public tool entry point used by function-calling backends."""
    return executor.execute("run_shell_command", {"command": command, "reason": reason})
