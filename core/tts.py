...
Downloads the matching Piper executable for the current platform.
"""
...
class PiperTTS:
    """Piper TTS wrapper using the platform's pre-built executable."""
    ...
    # Piper executable configuration
    PIPER_VERSION = "2023.11.14-2"
    
    @classmethod
    def get_piper_release_url(cls):
        system = platform.system().lower()
        machine = platform.machine().lower()
        
        if system == "windows":
            return f"https://github.com/rhasspy/piper/releases/download/{cls.PIPER_VERSION}/piper_windows_amd64.zip"
        elif system == "darwin": # macOS
            arch = "aarch64" if machine == "arm64" else "x86_64"
            return f"https://github.com/rhasspy/piper/releases/download/{cls.PIPER_VERSION}/piper_macos_{arch}.tar.gz"
        else: # Linux
            arch = "aarch64" if machine == "aarch64" else "x86_64"
            return f"https://github.com/rhasspy/piper/releases/download/{cls.PIPER_VERSION}/piper_linux_{arch}.tar.gz"
    ...
    def _download_piper_executable(self):
        ...
        piper_exe_dir = self.piper_dir / f"piper_{system}"
        ...
        try:
            r = http_session.get(self.get_piper_release_url(), stream=True)
            r.raise_for_status()
            ...
            # Extract
            file_data.seek(0)
            url = self.get_piper_release_url()
                ...
                with tarfile.open(fileobj=file_data, mode='r:gz') as tar:
                    for member in tar.getmembers():
                        if member.name.startswith("piper/"):
                            relative_name = Path(member.name[6:])
                            if relative_name and ".." not in relative_name.parts:
                                member.name = str(relative_name)
                                tar.extract(member, path=piper_exe_dir)
            
            # Make executable on Unix
            if not is_windows and piper_exe.exists():
