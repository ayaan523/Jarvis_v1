...
from config import FUNCTIONS, RESPONDER_MODEL, OLLAMA_URL, MAX_HISTORY
...
ACTION_FUNCTIONS = {"control_light", "set_timer", "set_alarm", "create_calendar_event", "add_task", "web_search", "run_shell_command", "focus_window", "list_windows", "close_window"}
...
class ChatWorker(QObject):
    ...
    def process(self):
        ...
        try:
            confirmation = function_executor.confirm_pending_shell_command(self.user_text)
            if confirmation is not None:
                self.toast.emit(confirmation["message"], confirmation["success"])
                self._generate_response_with_context("run_shell_command", confirmation)
                return

            os_request = detect_os_control_request(self.user_text)
            if os_request:
            ...
            self.status.emit(f"Using {backend.name}")
            if isinstance(backend, GeminiBackend):
                self._generate_gemini_response(backend)
                return
    ...
    def _generate_response_with_context(self, func_name: str, result: dict, enable_thinking: bool = False):
        ...
        try:
            for text_chunk, thought_chunk in stream_chat_response(self.messages, enable_thinking):
                if self.stop_event.is_set():
                    break
                    
                if thought_chunk:
                    self.thought_chunk.emit(thought_chunk)
    ...
    def _generate_gemini_response(self, backend):
        """Use Gemini's native function-calling loop without loading FunctionGemma."""
        self.messages.append({"role": "user", "content": self.user_text})
        self.ui_update.emit()
        self.status.emit("Gemini is responding...")
        self.think_start.emit(False)
        try:
            self.full_response = backend.generate(self.messages, FUNCTIONS, function_executor.execute)
            self.response_chunk.emit(self.full_response)
            if self.is_tts_enabled and not DEBUG_SKIP_TTS and self.full_response:
                tts.queue_sentence(self.full_response)
        except Exception as error:
            self.error.emit(f"Gemini API Error: {error}")
            self.full_response = ""
        self.think_end.emit()
        self.messages.append({"role": "assistant", "content": self.full_response})
        if self.current_session_id:
