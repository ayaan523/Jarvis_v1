    ...
    def collect(self):
        ...
        try:
            ...
            try:
                ...
                stats['active_backend'] = backend.name
            except Exception:
                stats['active_backend'] = "Gemini Flash"
...
class SystemMonitor(QFrame):
    ...
    def _on_stats_updated(self, stats):
        ...
        if active_backend:
            display_parts.append(active_backend)
        
        # Add Ollama models if loaded locally
        if isinstance(models, list) and models:
            for m in models:
                if m not in display_parts and m.lower() not in active_backend.lower():
                    display_parts.append(m)
        
        # Add local router if loaded
        if router_loaded and "gemma" not in [p.lower() for p in display_parts]:
