...
from config import (
    FUNCTIONS, RESPONDER_MODEL, OLLAMA_URL, MAX_HISTORY, ULTRA_LIGHT_MODE, GRAY, RESET, CYAN, GREEN, WAKE_WORD
)
from core.stt import STTListener
from core.llm import detect_os_control_request, route_query, should_bypass_router, http_session
...
from core.backends import GeminiBackend, select_backend

# Functions that are actions (not passthrough)
ACTION_FUNCTIONS = {
    "control_light", "set_timer", "set_alarm", 
    "create_calendar_event", "add_task", "web_search", "run_shell_command",
...
class VoiceAssistant(QObject):
    ...
    def initialize(self) -> bool:
        ...
        try:
            if ULTRA_LIGHT_MODE and isinstance(select_backend(), GeminiBackend):
                from core.gemini_live_audio import GeminiLiveAudio
                self.live_audio = GeminiLiveAudio()
                print(f"{CYAN}[VoiceAssistant] Gemini Live audio ready.{RESET}")
                return True
    ...
    def start(self):
        ...
        if self.live_audio:
            self.running = True
            threading.Thread(target=self._run_live_turn, daemon=True).start()
            return
    ...
    def _run_live_turn(self):
        try:
            self.live_audio.run_turn()
        except Exception as error:
            self.error_occurred.emit(f"Gemini Live audio error: {error}")
        finally:
            self.running = False
    ...
    def _on_speech(self, text: str):
        ...
        text = text.lower().replace("jarvis", "").replace("ada", "").strip()
        if not text:
            return
    ...
    def _process_query(self, user_text: str):
        ...
        try:
            confirmation = function_executor.confirm_pending_shell_command(user_text)
            if confirmation is not None:
                self._generate_response_with_context("run_shell_command", confirmation, user_text)
                return

            os_request = detect_os_control_request(user_text)
            if os_request:
                func_name, params = os_request
            ...
            if isinstance(backend, GeminiBackend):
                self._generate_gemini_response(backend, user_text)
                return
    ...
    def _generate_response_with_context(self, func_name: str, result: dict, user_text: str, enable_thinking: bool = False):
        ...
        try:
            ...
            # Stream response
            from core.hybrid_client import stream_chat_response
            try:
                for text_chunk, _ in stream_chat_response(self.messages, enable_thinking):
                    if text_chunk:
                        full_response += text_chunk
                        
                        # Queue for TTS
                        sentences = sentence_buffer.add(text_chunk)
                        for s in sentences:
                            tts.queue_sentence(s)
            except Exception as e:
                print(f"{GRAY}[VoiceAssistant] API Error: {e}{RESET}")
    ...
    def _generate_gemini_response(self, backend, user_text: str):
        """Use Gemini function calling directly for online voice interactions."""
        try:
            self.messages.append({"role": "user", "content": user_text})
            response = backend.generate(self.messages, FUNCTIONS, function_executor.execute)
            self.messages.append({"role": "assistant", "content": response})
            if response:
                tts.queue_sentence(response)
            self.processing_finished.emit()
        except Exception as error:
            self.error_occurred.emit(f"Gemini API Error: {error}")
            self.processing_finished.emit()
