...
from config import OPTIONAL_FEATURES_ENABLED, ULTRA_LIGHT_MODE, VOICE_ASSISTANT_ENABLED, VOICE_AUTO_START, GREEN, RESET
...
class MainWindow(FluentWindow):
    ...
    def __init__(self, listen_on_start=False):
        super().__init__()
    ...
    def _init_voice_assistant(self):
        ...
        if VOICE_ASSISTANT_ENABLED and (self.listen_on_start or VOICE_AUTO_START):
            ...
            def init_va():
                ...
                if voice_assistant.initialize():
                    ...
                    if not voice_assistant.live_audio:
                        tts.toggle(True)
            ...
            print("[App] Voice assistant is available; start with --listen for push-to-talk.")
    ...
    def _init_window(self):
        # Core tabs are lazy; optional integrations are not imported by default.
        ...
        if OPTIONAL_FEATURES_ENABLED:
            ...
            self.briefing_view = LazyTab(BriefingView, "briefingInterface")
    ...
    def closeEvent(self, event):
        ...
        if ULTRA_LIGHT_MODE and not self._quitting:
            self.hide()
            event.ignore()
            return
